...
from windows_controller import WindowsController  # 假设你有一个WindowsController类
# from telegram_notifier import send_notification
from WindowsGameWindow import WindowManager
from GameController import GameController

class TaskManager:
    def __init__(self, app_package, window_manager):
        self.WindowController = WindowsController(window_manager)  # 假设窗口大小为1920x1080
        self.GameController = GameController(self.WindowController)
        ...
        self.TaskConfig = {
            'Task_zdy_left': self.Task_zdy_left,
        }
        for name in self.TaskConfig.keys():
            self.task_next_run[name] = None
    ...
    def execute_task_by_name(self, task_name):
        ...
        if task_name in config['tasks'] and config['tasks'][task_name]['enabled']:
            ...
            cooldown_time = self.task_config["task_name"]()  # 默认冷却时间为600秒
    ...
    def ensure_app_running(self):
        ...
        if not self.controller.is_app_foreground(self.app_package):
            log(f"应用 {self.app_package} 不在前台，正在启动...")
            self.controller.launch_app(self.app_package)
    ...
    def execute_task(self, task_config):
        ...
        while i < len(task_config['actions']):
            ...
            if action['type'] == 'get_countdown':
                ...
                self.controller.tap(action['x'], action['y'])
                i += 1
    ...
    def close_app_for_idle(self):
        ...
        if not self.app_closed:
            ...
            self.controller.force_stop_app(self.app_package)
